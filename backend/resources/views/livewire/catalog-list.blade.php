<div class="container mx-auto">
    <div class="grid grid-cols-2 w-3/12">
        <h2 class="mt-4 text-3xl font-extrabold leading-none tracking-tight text-gray-900 md:text-4xl">Catalogs</h2>
        ...
        <table class="w-full text-sm text-left text-gray-700">
            ...
            @foreach($catalogs as $catalog)
                <tr class="odd:bg-white even:bg-gray-50 border-b border-gray-200">
                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ $catalog->name }}</td>
                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                        {{ $catalog->products->pluck('name')->join(', ') }}
                    </td>
                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                        <a href="{{ route('catalogs.edit', $catalog->id) }}" class="font-medium text-blue-600 hover:underline">Edit</a>
                    </td>
                </tr>
            @endforeach
            </tbody>
